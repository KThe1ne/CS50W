...
def getUserTrades():
    ...
    endpoint = "/api/v1/orders?tradeType=TRADE&pageSize=500"
...
def getGrowthPercentage(purchaseDetails, currPrice):
    ...
    totalInvestment = sum([purchaseDetail[0] for purchaseDetail in purchaseDetails])
    print(totalInvestment)
    growth = sum([(purchaseDetail[0]/totalInvestment)*((currPrice-purchaseDetail[1])/purchaseDetail[1]) for purchaseDetail in purchaseDetails])*100
    print(growth)


def pastHistoricalGrowth():

    currPrice = 800

    data = []
    klines = getPriceHistory("BTC-USDT", "4hour", "1M")["data"]
    data = [{
        "startTime": kline[0],
    ...
    while (prevDate >= lastDate):
        purchasePrice = df["avgPrice"].loc[df["startTime"] == str(int(datetime.datetime.timestamp(prevDate)))]
    ...
    # purchasePrices = [[100,1000],[100,2000],[100,3000]]
